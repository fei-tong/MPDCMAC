...
% Date: May 13, 2015
 ...
 function [ ] = process_data_ccp( separater )
...
%%
%file_in = 'data.txt';
protocol_array = char('xmac','contikimac','nullrdc');
disp(protocol_array)

pgi_array = [5 10 15 20 25]; %packet generation interval (unit: second)
input_data_folder = '.';
...
for i = pgi_array
        ...
        for j = 1:1:protocol_num
            protocol = protocol_array(j,:);
             if strcmp(protocol,'xmac      ') % if fa is 'fa ', the last space should be removed
                protocol='xmac';
             end
